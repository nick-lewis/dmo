...
def checkpoint_payload(session, event, state=None, client_ui_state=None):
    entry_state = set_runtime_current_event(dict(state or session.runtime_state or {}), event)
    return {
...
def checkpoint_summary(payload, mode):
    ...
    if slide_ref:
        context_label = f"{context_label}, slide={slide_ref}" if context_label else f"slide={slide_ref}"
...
def create_session(request):
    ...
    if checkpoint_id:
        ...
        if not checkpoint:
            return JsonResponse({"detail": "Saved state not found."}, status=404)
